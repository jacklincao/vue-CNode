  ...
  mutations: {
    ACCOUNT_AUTH_STATUS_CHANGED: (state, data) => {
      if (data.params.remember) {
        Vue.set(state.auth, 'accountPwd', data.params.accesstoken)
        window.sessionStorage.setItem(ACCOUNT_PWD, JSON.stringify(data.params))
      }
      Vue.set(state.auth, 'token', data.params.accesstoken)
      Vue.set(state.auth, 'user', data)
      Vue.set(state.auth, 'isLogin', true)
      window.sessionStorage.setItem(AUTH_TOKEN, data.params.accesstoken)
